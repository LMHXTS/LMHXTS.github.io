...
为了让你有更直观的感受，我们假设一个场景：根据身高和体重来预测一个人是“长跑运动员”（类别0）还是“普通大学生”（类别1）。你的体征数据是特征向量 $X = [x_1, x_2] = [181, 65]$（即身高181cm，体重65kg）。我们将利用高斯朴素贝叶斯（Gaussian NB）来计算你属于哪个类别的概率最大。
...
由于对于所有的类别 $c$，分母 $P(X)$ 都是相同的（即你的身高体重的客观存在概率），我们在比较大小时可以将其忽略，只求分子最大化：
...
因为身高和体重是连续变量，我们假设特征服从**高斯分布（正态分布）**。对于类别 $c$ 中的第 $i$ 个特征，其概率密度函数为：
...
为了让你理解矩阵运算与概率统计的结合，我们不使用 `sklearn`，而是纯手工用 `NumPy` 搭建一个高斯朴素贝叶斯分类器。
